...
export default function SuperAdminCoaches() {
  ...
  const { data: coaches, loading } = useApiCall(() => superAdminService.getAllCoaches(), []);
  ...
  return (
    <View style={styles.container}>
      <ScreenHeader 
        title="Global Coach Directory" 
        subtitle="All platform coaches"
        showBack={true}
      />
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        {loading ? (
          ...
          filteredCoaches.map((coach: any) => (
            <TouchableOpacity key={coach.id} style={styles.userCard}>
              <View style={styles.userLeft}>
                <AvatarRing size="md" name={coach.name} imageUri={coach.profilePhotoUrl} />
                <View style={styles.userInfo}>
                  <Text style={styles.userName}>{coach.name}</Text>
                  ...
                  <View style={styles.detailRow}>
                    ...
                    <Text style={styles.detailText}>{coach.gym?.name || 'No Gym Assigned'}</Text>
                  </View>
                </View>
              </View>
